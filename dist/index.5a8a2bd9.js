...
const formatTimer = ()=>{
    ...
    if (isCountdown) return Math.ceil(-adjustedTime / 1000).toString();
    else {
...
const updateTimerDisplay = ()=>{
    if (timer == null || timer.state == "Reset") timerDisplay.innerHTML = "00:00.00";
...
infoButton.addEventListener("click", (e)=>{
    ...
    if (settings.showDebug) fetch("/api/disable_debug", {
        method: "POST"
    });
    else fetch("/api/enable_debug", {
        method: "POST"
    });
});
const frameCallback = ()=>{
